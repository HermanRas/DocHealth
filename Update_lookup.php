...
<html>
    ...
    if (isset($_POST["CompNr"]) && !empty($_POST["CompNr"])) {
        ...
        $sql_EmployeeData = "select [OCHMedical].[dbo].[tWPatientDetails].[CompanyNumber],[NameSurname],[Gender] from [OCHMedical].[dbo].[tWPatientDetails] WHERE [CompanyNumber] = '$CompNr';";
        $Employee_list = odbc_exec($conn, $sql_EmployeeData);
            //link ODBC Values
            $EmpID = odbc_result($Employee_list, "CompanyNumber");
            $NameSurname = odbc_result($Employee_list, "NameSurname");
    ...
    <div class="container">  
      ...
      <form id="contact" action="Update_EmployeeData.php" method="post">
        <h3>Update Employee Health </h3>
        <h4>Please Confirm Details</h4>
        <fieldset>
            Name: <input name="empNameSurname" type="text" value="<?php echo $NameSurname;?>" readonly="readonly" required>
